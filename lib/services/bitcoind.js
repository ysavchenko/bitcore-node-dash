...
Bitcoin.prototype.govObjectList = function(options, callback) {
  ...
  this.client.gobject('list', function(err, response) {
    ...
    for (var i = 0; i < gobjects.length; i++) {
      ...
      govObject.prototype = govObject.Proposal.prototype;

      var proposal = new govObject(response.result[gobjects[i]].DataHex);
...
Bitcoin.prototype.govObjectHash = function(hash, callback) {
  ...
  this.client.gobject('get', hash, function(err, response) {
      ...
      var proposal = new govObject(response.result.DataHex);

      if (proposal.serialize() === response.result.DataHex) { // serialize proposal back to Hex to verify it's consistent with RPC
          result.push({
