...
Bitcoin.prototype.sendTransaction = function(tx, options, callback) {
  ...
  // TODO: move this to a different function, or pass an option that determines if this tx should be passed to :9999 using RPC or P2P

  // TODO: evaluate if this tx meets criteria for InstantSend: CIn >= 6 confirmations, fee > 0.001

  // TODO: determine network (mainnet or testnet) and set network for Messages
  ...
  var ixMessage = messages.TXLockRequest(transaction);

  var peer = new Peer({ host: '127.0.0.1', network: 'testnet' });

  peer.on('ready', function() {

      peer.sendMessage(ixMessage);

      // TODO: need to set up a ZMQ Notification that alerts of Transaction Lock Failure

  });

  self.on('tx', function(message) { // wait for ZMQTransactionHandler to emit tx

      var tx = new Transaction(message);
      var txObj = tx.toObject();

      if (txObj.hash === transactionObj.hash) { // confirm emitted tx matches our transaction

          // TODO: get TXLock details (e.g. txlock = true / false)

          callback(null, txObj.hash); // perform callback

      }

  });

  peer.connect(); // TODO: disconnect peer after request is completed.

  /*
  self.client.sendRawTransaction(tx, allowAbsurdFees, function(err, response) {
      if (err) {
          return callback(self._wrapRPCError(err));
      }
      callback(null, response.result);
